...
The additive `pi-subagents/managed-dispatch` subpath exports the managed-dispatch v1 public contract. For an active persisted parent session, the extension opens the private managed journal, performs a bounded deterministic recovery pass, and only then advertises a live `ready` capability for `spawn`, exact `resume`, `status`, and `details`. Steering, interrupt, stop, and retirement remain disabled. Journal ownership conflict, corruption, excessive recovery scale, session replacement, or shutdown leaves managed dispatch unavailable without affecting the ordinary `subagent` tool or RPC.

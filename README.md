...
Exact managed resume has an inert authorization foundation but remains unavailable at runtime. The foundation accepts only a closed exact index-`0` request, resolves only a same-consumer managed terminal actor, revalidates committed admission plus exact resumable lifecycle proof, requires the retained canonical JSONL to have a free lease, binds the recovery descriptor and source proof into a resume-specific contract/profile, and reserves immutable journal fields for source and future lease admission. No provider resume listener, prepared resume runner, or resume capability is enabled by this foundation.

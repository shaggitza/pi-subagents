...
The additive `pi-subagents/managed-dispatch` subpath exports the managed-dispatch v1 public contract. For an active persisted parent session, the extension opens the private managed journal, performs a bounded deterministic recovery pass, and only then advertises live `ready` capabilities for `spawn`, exact `resume`, `steer`, `interrupt`, `stop`, `retire`, `status`, and `details`. Journal ownership conflict, corruption, excessive recovery scale, session replacement, or shutdown revokes every managed mutation capability without affecting the ordinary `subagent` tool or RPC.

Non-launching spawn preflight still resolves host-owned agent and skill content plus every planned write/evidence root without creating child/session/run state. Ready spawn requests use the guarded prepared executor, durable operation identity, execution-time contract/root recomputation, and the token-bound ready → proceed → accepted → commit → committed handshake. Startup and read-time reconciliation never relaunch post-boundary ambiguity or republish a control command after its publication boundary. Exact consumed-plus-acknowledgment evidence terminalizes controls; missing, corrupt, or consumed-without-acknowledgment transport becomes sticky `uncertain`. Retirement durably tombstones only safe terminal, failed-before-launch, or explicitly acknowledged uncertain actors, and recovery deterministically completes a half-finished retirement saga. Status/details resolve exact operation or run identity only within the active parent and consumer namespace; they expose bounded method, target/actor/run, control outcome, retirement identity, lifecycle-v3 proof, canonical child-session identity, and child index `0`, but not executor request bodies or steer messages, private paths, raw admission tokens, or token digests. Only exact durable `observed` lifecycle proof correlated to the operation, runner instance, admission token digest, and canonical session can terminalize a launch journal record.

Prepared managed spawn and exact resume run in a host-only configured-runtime mode: the exact opaque task is carried in a canonical trim-stable print-mode stdin envelope instead of CLI arguments or `@file` expansion, then decoded by the configured runtime without the ordinary `Task:` wrapper. Acceptance/output prompt decoration is omitted, ambient extensions and the ordinary prompt/fanout runtimes are disabled, and only the configured attested extension set is loaded. Managed fanout is rejected even if a malformed profile attempts to request it. The dedicated runner exits only after its output stream finishes and final artifact/lease cleanup completes; the parent retains its close observer until lifecycle-v3 publication. Managed result handoffs do not emit ordinary path-bearing `subagent-notify` messages into the parent session. These changes are selected only by the private prepared-admission path and do not alter ordinary `subagent` execution.

Exact managed resume accepts only a closed exact index-`0` request, resolves only a same-consumer managed terminal actor, revalidates committed admission plus exact resumable lifecycle proof, and reuses the retained canonical JSONL under one exclusive revival lease. Terminal evidence binds the session file's device/inode as well as its canonical path identity, so same-path replacement is unavailable as resume authority. One current host agent configuration—fallbacks, tools/extensions/MCP, prompt/inheritance, skill paths and resolved content, and memory—drives both contract preflight and the runner; the retained descriptor supplies the exact session/output/artifact roots and bounded run state, with its ceiling intersected with the active-session ceiling. A fixed candidate is bound through every result/config/admission/output/artifact root; the combined runner acquires the canonical lease before token-bound ready → proceed → accepted → commit → committed admission, and model/session execution remains blocked until journal acceptance. Exact released-lease lifecycle proof can terminalize the resume operation, while every ambiguous post-boundary failure remains sticky `uncertain` and never relaunches. A provider restart explicitly treats accepted state without durable close proof as observer-loss uncertainty rather than liveness or terminal evidence. Spawn and resume preflight are served exactly once by the consolidated provider. Each successful result includes a host-derived, fixed-shape `childCapability` projection containing only effective builtin-plus-MCP tool count, runtime/configured extension counts and content-set digests, ambient-extension disablement, and fanout authorization; it does not expose tool or extension names, paths, content, prompts, tasks, or caller material. A zero-extension set projects as `null`; otherwise each digest is 64 lowercase SHA-256 hex. Consumers can call `computeManagedExtensionSetDigest(extensionFilePaths, cwd?)` from `pi-subagents/managed-dispatch` to hash their own shipped bridge with the same bounded, no-follow regular-file and domain-separated sorted-digest procedure, and can strictly validate the fixed projection with `parseManagedChildCapabilityV1`. Resume capability is revoked immediately outside recovery-complete `ready` state. Ordinary resume behavior is unchanged.

Reserved transport constants are `subagents:managed-dispatch:v1:request` and `subagents:managed-dispatch:v1:reply:`; ordinary RPC remains unchanged. A managed `consumerId` is only a namespace component, not authentication. Process-local event-bus providers assume trusted installed extensions: the bus provides no confidentiality, authentication, or isolation between extensions. Extension content-set digests attest the descriptor snapshots read during preflight; they do not hold files open through child startup or claim to prevent later same-user content mutation. Managed mutation still recomputes the exact launch contract at its final authorization boundary. Managed root fences use no-follow descriptors, inode identity, and a synchronous check immediately before effects, but Node does not expose portable `openat`-relative creation; hostile same-UID filesystem mutation after that check is outside this single-host/same-user trust boundary and is not claimed to be prevented.
...
Lifecycle artifact v3 adds `process-terminal-candidate.json` (private runner evidence) and `process-terminal.json` (the public proof projection). A proof is `observed` only after the live parent observes the exact detached runner's `close` event, every recorded child writer has a close record, and any tracked canonical-session lease is free. Prepared managed runs also require a strict journal/request/admission binding and exact fresh canonical-session projection; the raw admission token is never included. If the observer is unavailable, the proof is `unknown`; do not infer process exit from `endedAt`, result-file existence, PID disappearance, or lease-directory absence. The `subagent:process-terminal` event and RPC `ping.capabilities.processTerminalProof` expose this status. Process proof is point-in-time evidence and remains separate from execution success or stopped/non-resumable state.

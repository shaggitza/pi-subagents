...
Exact managed resume now has an unregistered execution gate but remains unavailable through the provider. It accepts only a closed exact index-`0` request, resolves only a same-consumer managed terminal actor, revalidates committed admission plus exact resumable lifecycle proof, and reuses the retained canonical JSONL under one exclusive revival lease. A fixed preflight candidate is bound through result/config/admission roots; the combined runner acquires the canonical lease before token-bound ready → proceed → accepted → commit → committed admission, and model/session execution remains blocked until journal acceptance. Exact released-lease lifecycle proof can terminalize the resume operation, while every ambiguous post-boundary failure remains sticky `uncertain` and never relaunches. No provider resume listener or resume capability is enabled by this gate, and ordinary resume behavior is unchanged.

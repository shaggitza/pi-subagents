...
The additive `pi-subagents/managed-dispatch` subpath exports the managed-dispatch v1 public contract. For an active persisted parent session, the extension now opens the private managed journal, performs a bounded deterministic recovery pass, and only then advertises a live `ready` capability for `spawn`, `status`, and `details`. Capability replies explicitly keep exact resume, steering, interrupt, stop, and retirement disabled. Journal ownership conflict, corruption, excessive recovery scale, session replacement, or shutdown leaves managed dispatch unavailable without affecting the ordinary `subagent` tool or RPC.

Non-launching spawn preflight still resolves host-owned agent and skill content plus every planned write/evidence root without creating child/session/run state. Ready spawn requests use the guarded prepared executor, durable operation identity, execution-time contract/root recomputation, and the token-bound ready → proceed → accepted → commit → committed handshake. Startup and read-time reconciliation never relaunch post-boundary ambiguity. Status/details resolve exact operation or run identity only within the active parent and consumer namespace; they expose bounded state, lifecycle-v3 proof, canonical child-session identity and child index `0`, but not executor request bodies, private paths, raw admission tokens, or token digests. Only exact durable `observed` lifecycle proof correlated to the operation, runner instance, admission token digest, and canonical session can terminalize the journal.

Exact managed resume now has an unregistered execution gate but remains unavailable through the provider. It accepts only a closed exact index-`0` request, resolves only a same-consumer managed terminal actor, revalidates committed admission plus exact resumable lifecycle proof, and reuses the retained canonical JSONL under one exclusive revival lease. Terminal evidence binds the session file's device/inode as well as its canonical path identity, so same-path replacement is unavailable as resume authority. One immutable recovered execution specification—fallbacks, tools/extensions/MCP, prompt/inheritance, skill paths and resolved content, memory, output/artifact roots, plus the retained ceiling intersected with the active-session ceiling—drives both contract preflight and the runner. A fixed candidate is bound through every result/config/admission/output/artifact root; the combined runner acquires the canonical lease before token-bound ready → proceed → accepted → commit → committed admission, and model/session execution remains blocked until journal acceptance. Exact released-lease lifecycle proof can terminalize the resume operation, while every ambiguous post-boundary failure remains sticky `uncertain` and never relaunches. A provider restart explicitly treats accepted state without durable close proof as observer-loss uncertainty rather than liveness or terminal evidence. No provider resume listener or resume capability is enabled by this gate, and ordinary resume behavior is unchanged.

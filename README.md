...
The additive `pi-subagents/managed-dispatch` subpath exports the managed-dispatch v1 contract foundation. The extension currently registers only a non-launching spawn-preflight listener: it validates the complete ordinary executor request, resolves host-owned agent and skill content plus every planned write/evidence root, and creates no child/session/run directory. On first successful preflight it may persist one non-secret host-ID file for stable single-host binding. An unregistered prepared-spawn executor seam can bind one candidate across async, result-reservation, config, canonical-session, and runner-admission paths behind a final pre-side-effect callback. A token-bound ready → proceed → accepted → commit → committed handshake keeps the runner blocked before model/session execution while future journal callbacks record ready and accepted state. The parent waits for durable committed evidence before reporting launch admission, but no managed mutation listener calls it. Mutation methods and managed runtime capability advertisement remain unavailable. A private durable operation journal provides single-host ownership, session/consumer/operation namespaces, immutable request identity, replay, fenced crash states, and atomic runner-instance/admission-token-digest correlation. An unregistered coordinator now binds that journal to execution-time contract recomputation, an inode-aware prepared-spawn boundary, admission callbacks, committed-evidence replay, and conservative uncertain recovery. Managed prepared runs additionally bind lifecycle-v3 proof to the parent/consumer/operation/request/candidate, runner instance, admission-token digest, authorized async root, and canonical child session. Only exact durable `observed` proof can terminalize the journal, including after restart; missing, malformed, mismatched, or `unknown` evidence never relaunches and remains fail-closed. The registered listener still does not call the coordinator. Exported requirements are static protocol semantics, not capabilities. Reserved transport constants are `subagents:managed-dispatch:v1:request` and `subagents:managed-dispatch:v1:reply:`; ordinary RPC remains unchanged. A managed `consumerId` is only a namespace component, not authentication. Process-local event-bus providers assume trusted installed extensions: the bus provides no confidentiality, authentication, or isolation between extensions. Managed root fences use no-follow descriptors, inode identity, and a synchronous check immediately before effects, but Node does not expose portable `openat`-relative creation; hostile same-UID filesystem mutation after that check is outside this single-host/same-user trust boundary and is not claimed to be prevented.

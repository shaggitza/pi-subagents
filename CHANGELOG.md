...
### Added
- Added an unregistered exact managed-resume execution gate: a fixed candidate reuses the exact source JSONL, combines canonical-session lease ownership with correlated prepared runner admission, binds runner/admission/lease identities durably, joins retries, terminalizes only from exact released-lease lifecycle proof, and never relaunches ambiguous post-boundary states. Provider resume and capability advertisement remain disabled, and ordinary resume is unchanged.
- Added an unregistered exact managed-resume authorization foundation: a closed index-0 executor request, exact same-parent/same-consumer terminal source inspection, committed admission and lifecycle-v3 proof checks, free canonical JSONL lease verification, retained recovery-descriptor identity, a resume-specific host/profile/contract resolver, and immutable journal source/lease correlation. The live provider continues to advertise resume as unavailable until the prepared-resume runner/coordinator gate is complete.
- Registered a recovery-gated managed-dispatch provider for capability discovery, durable spawn, exact status, and bounded details. Readiness is advertised only after active-parent journal ownership and bounded startup recovery; resume and control capabilities remain explicitly false. Status/details expose sanitized lifecycle-v3 and canonical child identity without private paths, request bodies, or admission material, while managed provider failure leaves ordinary tools and RPC intact.

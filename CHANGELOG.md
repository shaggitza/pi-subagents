...
### Added
- Added host-only managed child runtime composition for prepared spawn and exact resume. Managed runners preserve exact opaque tasks through a canonical trim-stable stdin envelope rather than CLI or `@file` parsing, load only the configured attested extension set, disable ambient extensions and ordinary prompt/fanout runtimes, reject fanout authorization, and omit model-facing acceptance/output task decoration. Ordinary `subagent` launches remain unchanged.
- Added a strict bounded `childCapability` projection to successful managed spawn and resume preflight results. It attests only effective builtin-plus-MCP tool count, runtime/configured extension counts and privacy-safe content-set digests, ambient-extension disablement, and fanout authorization from the exact resolved launch contract; names, paths, content, prompts, tasks, and caller material remain private. The public helper computes matching bounded no-follow regular-file set digests for consumer-shipped bridges, zero-extension sets remain `null`, and a strict parser validates the fixed projection.
- Activated recovery-gated managed `steer`, `interrupt`, `stop`, and `retire` through the consolidated event-bus provider. Command publication is one-shot and session-fenced; startup/read reconciliation never republishes, exact consumed-plus-ack evidence terminalizes, ambiguous transport remains sticky uncertain, and half-completed retirement finishes deterministically. Capabilities are true only after bounded journal recovery, status/details expose only bounded command and retirement identity, and ordinary controls, RPC, and tools are unchanged.
- Activated recovery-gated exact managed resume through the consolidated managed-dispatch provider. Spawn and resume preflight now have exactly one responder; ready capability truthfully enables exact resume only after journal recovery and revokes it on reset, shutdown, or unavailable ownership. Resume mutation, status/details, terminal replay, session-generation fencing, and startup observer-loss recovery dispatch by durable operation method without changing ordinary resume.
- Added an exact managed-resume execution gate: a fixed candidate reuses the exact source JSONL, combines canonical-session lease ownership with correlated prepared runner admission, binds runner/admission/lease identities durably, joins retries, terminalizes only from exact released-lease lifecycle proof, and never relaunches ambiguous post-boundary states. Ordinary resume is unchanged.
- Added an exact managed-resume authorization foundation: a closed index-0 executor request, exact same-parent/same-consumer terminal source inspection, committed admission and lifecycle-v3 proof checks, free canonical JSONL lease verification, retained recovery-descriptor identity, a resume-specific host/profile/contract resolver, and immutable journal source/lease correlation.
- Registered a recovery-gated managed-dispatch provider for capability discovery, durable spawn and exact resume, exact status, and bounded details. Readiness is advertised only after active-parent journal ownership and bounded startup recovery. Status/details expose sanitized lifecycle-v3 and canonical child identity without private paths, request bodies, or admission material, while managed provider failure leaves ordinary tools and RPC intact.
- Bound prepared managed lifecycle-v3 proof to the durable parent/consumer/operation/request/candidate identity, runner instance, admission-token digest, authorized async root, and exact canonical child session. The managed coordinator now terminalizes only from strict durable `observed` proof, including restart replay, while malformed, missing, mismatched, and `unknown` evidence stays fail-closed without relaunch. The raw admission token is not projected.
- Added a managed spawn coordinator that joins concurrent retries, recomputes and compares the host-owned contract at the final dispatch boundary, atomically advances the durable operation journal through runner admission, validates committed evidence against runner-instance and token-digest correlation, and converts ambiguous recovery to `uncertain` without relaunching.
- Added a prepared-spawn executor seam for managed dispatch: one preflight candidate binds async, result-reservation, runner-config, canonical-session, and runner-admission paths; a final callback runs before per-launch filesystem/process side effects; inherited/nested routing is rejected and rechecked; result publication is exclusively reserved; and a token-bound ready → proceed → accepted → commit → committed handshake keeps the runner blocked before model/session execution while host callbacks durably record admission and the parent waits for durable commit evidence. Ordinary execution remains unchanged.
- Added the host-neutral `pi-subagents/managed-dispatch` v1 contract foundation with strict canonical JSON, identifiers, semantic digests, and static single-host/fail-closed requirements vocabulary, plus a non-launching spawn-preflight listener and shared resolver that derive agent/skill-content and root identity without creating child/session/run state (preflight may lazily persist one stable non-secret host-ID file); a private single-host durable operation journal supports atomic runner-instance and admission-token-digest correlation.
...
### Fixed
- Made dedicated async runner processes exit deterministically after final artifact and lease cleanup, while retaining a strong parent-side close observer until lifecycle-v3 proof publication. Managed result handoffs are consumed without ordinary `subagent-notify` messages, preventing child output and private session paths from entering the managed parent transcript.
- Hardened inert exact managed resume so its contract and runner share one current host agent configuration, including resolved skills, while retained session/artifact authority is intersected with the current capability ceiling; resume-as-source admission/lease identity and terminal session device/inode are revalidated, same-path JSONL replacement is rejected, and startup observer loss becomes durable uncertainty without relaunch.
- Closed the managed final-fence same-path substitution race by rechecking the original ancestor's real path, device, inode, and expected file kind immediately before launch side effects.

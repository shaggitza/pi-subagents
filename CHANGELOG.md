...
### Added
- Added a strict bounded `childCapability` projection to successful managed spawn and resume preflight results. It attests only effective builtin-plus-MCP tool count, runtime/configured extension counts, ambient-extension disablement, and fanout authorization from the exact resolved launch contract; names, paths, prompts, tasks, and caller material remain private.
